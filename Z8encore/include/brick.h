...
The height of the brick is the first four bits and can be extracted with the macro BRICK_HEIGHT
The width of the brick is the last four bits and can be extracted with the macro BRICK_HEIGHT
A size can be made by a width and height by using the macro MAKE_SIZE.


data:
Contains wether the brick is indestructible, moving, its direction and its health. The first
bit is not used for anything.

indestructible:
Is the second bit. if this bit is set to 1 then the brick is indestructible which means it can't be killed.
...
is moving:
Is the third bit and determines if this brick is able to move. Set to 1 if the brick cshould be able to move.
...
health:
The last four bits are reserved for the bricks health which can max be 15.
